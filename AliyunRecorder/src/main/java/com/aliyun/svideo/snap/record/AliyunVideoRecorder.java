...
public class AliyunVideoRecorder extends Activity implements View.OnClickListener, View.OnTouchListener, ScaleGestureDetector.OnScaleGestureListener, GestureDetector.OnGestureListener {
    ...
    public static final String OUTPUT_PATH_URI = "output_path_uri";
    ...
    private final int SAVE_VIDEO_COMPLEGE = 1001;
    private final Handler videoHandler = new Handler() {
        public void handleMessage(Message msg) {

            switch (msg.what) {

                case SAVE_VIDEO_COMPLEGE:

                    mClipManager.deleteAllPart();
                    Bundle bundle = msg.getData();
                    Intent intent = new Intent();
                    String recordVideoPath=bundle.getString(OUTPUT_PATH);
                    String recordVideoUri=bundle.getString(OUTPUT_PATH_URI);
                    scanFile(recordVideoPath);
                    intent.putExtra(OUTPUT_PATH, recordVideoPath);
                    intent.putExtra(OUTPUT_PATH_URI,recordVideoUri);
            ...
            public void onFinish(final String outputPath) {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                    ...
                    ThreadUtils.runOnSubThread(new Runnable() {
                        @Override
                        public void run() {
                            UriUtils.saveVideoToMediaStore(AliyunVideoRecorder.this.getApplicationContext(), outputPath,videoHandler);
                        }
                    });
                } else {
                    scanFile(outputPath);
