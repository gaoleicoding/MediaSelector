...
public class AliyunVideoCropActivity extends Activity implements TextureView.SurfaceTextureListener,
    ...
    public final static int VIDEO_CROP_REQUEST_CODE = 167;
    ...
    private VideoQuality quality = VideoQuality.SD;
    ...
    private int maxCropDuration = 15000;
    ...
    private void getData() {
        ...
        try {
            ...
            if (duration >= maxCropDuration + 1000) {
                ToastUtil.showToast(this, R.string.alivc_crop_video_max_time);
            }
        } catch (Exception e) {
    ...
    private void initView() {
        ...
        int maxDiff = (int) (maxCropDuration / (float) duration * 100) + 1;
        seekBar.setProgressMinDiff(Math.min(minDiff, 100));
        seekBar.setProgressMaxDiff(Math.min(maxDiff, 100));
        ...
        dirationTxt.setText((int) ((duration > maxCropDuration ? maxCropDuration : duration) / 1000) + "s");
    ...
    public void onVideoSizeChanged(MediaPlayer mp, int width, int height) {
        ...
        if (crop != null && mEndTime == 0) {
            try {
                ...
                mEndTime = mEndTime > maxCropDuration ? maxCropDuration : mEndTime;
            } catch (Exception e) {
                ToastUtil.showToast(this, R.string.aliyun_video_crop_error);
            }
        }
            ...
            public void run() {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                    ...
                    ThreadUtils.runOnSubThread(new Runnable() {
                        @Override
                        public void run() {
                            UriUtils.saveVideoToMediaStore(AliyunVideoCropActivity.this.getApplicationContext(), outputPath, null);
                        }
                    });
                } else {
                    scanFile();
                ...
                intent.putExtra(CropKey.RESULT_KEY_DURATION, mEndTime - mStartTime);
                intent.putExtra(CropKey.RESULT_KEY_FILE_PATH, outputPath);
                //裁剪之后的跳转
                String tagClassName = AliyunSvideoActionConfig.getInstance().getAction().getTagClassName(ActionInfo.SVideoAction.CROP_TARGET_CLASSNAME);
                if (tagClassName == null) {
//                    try {
//                        MediaScannerConnection.scanFile(AliyunVideoCropActivity.this, new String[]{outputPath}, new String[]{"video/mp4"}, new MediaScannerConnection.OnScanCompletedListener() {
//                            @Override
...
//                    }
    ...
    private VideoSliceSeekBar.SeekBarChangeListener mSeekBarListener = new VideoSliceSeekBar.SeekBarChangeListener() {
        ...
        public void seekBarValueChanged(float leftThumb, float rightThumb, int whitchSide) {
            playHandler.removeMessages(PLAY_VIDEO);
            long seekPos = 0;
            mStartTime = (long) (duration * leftThumb / 100);
            mEndTime = (long) (duration * rightThumb / 100);
            int length = (int) (mEndTime - mStartTime);
            if (length > maxCropDuration) {
                mEndTime = mEndTime - length + maxCropDuration;
            }

            dirationTxt.setText((int)(mEndTime - mStartTime) / 1000 + "s");
            if (mPlayer != null) {
                mPlayer.seekTo((int) mStartTime);
            }
            playHandler.sendEmptyMessage(PLAY_VIDEO);
